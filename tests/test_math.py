...
class TestMath(unittest.TestCase):
    def test_array(self):
        ...
        class ArrayOpts(properties.HasProperties):
            ...
            myarraycomplex = properties.Array(
                'my complex array', dtype=complex
            )
        ...
        assert properties.Array.to_json(np.array([[0., 1., np.nan, np.inf]])
                                        ) == [[0., 1., 'nan', 'inf']]
        ...
        self.assertEqual(
            arrays.serialize(include_class=False), {
                'myarray': [0., 1., 2.],
    ...
    def test_vector2(self):
        ...
        assert isinstance(
            properties.Vector2.from_json([5., 6.]), vmath.Vector2
        )

        with self.assertRaises(ZeroDivisionError):
            hv2.vec2 = [0., 0.]

        assert properties.Vector2('').equal(
            vmath.Vector2(1., 2.), vmath.Vector2(1., 2.)
        )
        assert not properties.Vector2('').equal(
            vmath.Vector2(1., 2.), np.array([1., 2.])
        )
    ...
    def test_vector3(self):
        ...
        assert isinstance(
            properties.Vector3.from_json([5., 6., 7.]), vmath.Vector3
        )

        assert properties.Vector3('').equal(
            vmath.Vector3(1., 2., 3.), vmath.Vector3(1., 2., 3.)
        )
        assert not properties.Vector3('').equal(
            vmath.Vector3(1., 2., 3.), np.array([1., 2., 3.])
        )
