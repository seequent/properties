...
class StrPrimitive(properties.PropertyClass):
    ...
    vowel = properties.String("vowels", choices={'VOWEL': ('A','E','I','O','U'), 'MAYBE': 'Y'})
...
class APrimitive(properties.PropertyClass):
    opacity = properties.Range("My range", default=0.1, minValue=0., maxValue=1., required=True)
...
class TestBasic(unittest.TestCase):
    ...
    def test_color(self):
        ...
        self.assertRaises(ValueError, lambda: setattr(opts,'color', 5))
    ...
    def test_range(self):
        ...
        self.assertRaises(ValueError, lambda: setattr(opts,'opacity', 5))
        self.assertRaises(ValueError, lambda: setattr(opts,'opacity', -1))
        self.assertRaises(ValueError, lambda: setattr(opts,'opacity', [.25, .75]))
        self.assertRaises(ValueError, lambda: setattr(opts,'opacity', 'Kinda see-through'))
    ...
    def test_inheritance(self):
        ...
        self.assertRaises(ValueError, lambda: setattr(S.opts, 'color', 'SunburnRed'))
        self.assertRaises(ValueError, lambda: setattr(S.opts, 'opacity', 5))
